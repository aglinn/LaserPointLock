...
if __name__ == "__main__":
    ...
    from PyQt5.QtCore import pyqtSlot, pyqtSignnal, QThreadPool
    ...
    # Handle threading of cameras:
    cam1_thread = None
    cam2_thread = None
    updatemanager_thread = None
    ...
    def update_cam1_settings():
        ...
        global cam1_thread, cam_init_dict, threadpool
        if int(ui.cb_SystemSelection.currentIndex()) == 1:
            ...
            #Apply all updates:
            if cam1_thread is None:
                # Istantiate cam1_thread
                cam1_thread = CameraThread(cam_init_dict[str(ui.cb_cam_1.currentData())])
                #Connect signals
                cam1_thread.update_frame_signal.connect(update_image)
                # update camera settings
                cam1_thread.cam.setup_camera_for_image_acquisition()
                cam1_thread.cam.set_gain(cam1_gain)
                cam1_thread.cam.set_exposure_time(cam1_exp_time)
                cam1_thread.cam.update_frame()
                # update GUI with response
                ui.le_cam1_exp_time.setText('%.2f' % (cam1_thread.cam.exposure_time))
                ui.le_cam1_gain.setText('%.2f' % (cam1_thread.cam.gain / 8))
                cam1_reset = True
                resetHist(ui.gv_camera1)
                #Finally start the thread
                # See priority options here: https://doc.qt.io/qt-6/qthread.html#Priority-enum
                threadpool.start(cam1_thread, priority=5)
            else:
    ...
    def update_motors():
        ...
        if motor1_index != motor2_index:
            ...
            if "MDT693B" in ui.cb_motors_1.currentData(0):
                ...
                motor_list.append(
                    MDT693A_Motor(resourceManager, com_port=ui.cb_motors_1.currentData(0), ch1='X', ch2='Y'))
            if "MDT693B" in ui.cb_motors_2.currentData(0):
                motor_list.append(MDT693B_Motor(str(ui.cb_motors_2.currentData(0)[2:15])))
            else:
                motor_list.append(
                    MDT693A_Motor(resourceManager, com_port=ui.cb_motors_2.currentData(0), ch1='X', ch2='Y'))
    ...
    def find_cameras():
        ...
        if int(ui.cb_SystemSelection.currentIndex()) == 1:
            ...
            cam_model.appendRow(QtGui.QStandardItem(cam_key))
            cam_init_dict[cam_key] = 0
            c.close()
            c = BlackflyS(1)
            cam_key = 'blackfly s: ' + c.serial_no
            cam_model.appendRow(QtGui.QStandardItem(cam_key))
            cam_init_dict[cam_key] = 1
            c.close()
