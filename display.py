...
if __name__ == "__main__":
    ...
    self.ui.gv_camera1.addItem(Cam1_LeftArrow)
    self.ui.gv_camera1.addItem(Cam1_RightArrow)
    self.ui.gv_camera1.addItem(Cam1_DownArrow)
    self.ui.gv_camera1.addItem(Cam1_UpArrow)
    self.ui.gv_camera2.getView().addItem(Cam2_LeftArrow)
    self.ui.gv_camera2.getView().addItem(Cam2_RightArrow)
    self.ui.gv_camera2.getView().addItem(Cam2_DownArrow)
    self.ui.gv_camera2.getView().addItem(Cam2_UpArrow)
    ...
    cam1_x_plot = self.ui.gv_cam_xy.addPlot(row=0, col=0, labels={'left': 'Cam 1 X'}).plot()
    cam1_y_plot = self.ui.gv_cam_xy.addPlot(row=1, col=0, labels={'left': 'Cam 1 Y'}).plot()
    cam2_x_plot = self.ui.gv_cam_xy.addPlot(row=2, col=0, labels={'left': 'Cam 2 X'}).plot()
    cam2_y_plot = self.ui.gv_cam_xy.addPlot(row=3, col=0, labels={'left': 'Cam 2 Y'}).plot()
    # Initialize global variables for piezo motor voltages
    motor1_x = np.zeros(1)
    motor1_y = np.zeros(1)
    motor2_x = np.zeros(1)
    motor2_y = np.zeros(1)
    motor1_x_plot = self.ui.gv_piezo.addPlot(row=0, col=0).plot()
    motor1_y_plot = self.ui.gv_piezo.addPlot(row=1, col=0).plot()
    motor2_x_plot = self.ui.gv_piezo.addPlot(row=2, col=0).plot()
    motor2_y_plot = self.ui.gv_piezo.addPlot(row=3, col=0).plot()
    ...
    self.ui.gv_camera1.setColorMap(cmap)
    self.ui.gv_camera2.setColorMap(cmap)
    ...
    self.ui.gv_camera1.addItem(cam1_x_line)
    self.ui.gv_camera1.addItem(cam1_y_line)
    self.ui.gv_camera2.getView().addItem(cam2_x_line)
    self.ui.gv_camera2.getView().addItem(cam2_y_line)
    ...
    # LockTimeStart = 0 ---> Moved to Locked State
    ...
    def GUI_update_std(std):
        self.ui.le_cam1_dx_std.setText("{:.5f}".format(std[0]))
        self.ui.le_cam1_dy_std.setText("{:.5f}".format(std[1]))
        self.ui.le_cam2_dx_std.setText("{:.5f}".format(std[2]))
        self.ui.le_cam2_dy_std.setText("{:.5f}".format(std[3]))
    ...
    def take_img_calibration(cam_index, cam_view=0, threshold=0):
        ...
        if cam_view == 0:
            self.ui.le_cam1_max.setText(str(np.max(img/avg_frames))) #Just updates the GUI to say what the max value of the camera is
        else:
            self.ui.le_cam2_max.setText(str(np.max(img/avg_frames))) #Just updates the GUI to say what the max value of the camera is
    ...
    def take_img(cam_index, cam_view=0, threshold=0, resetView=False):
        ...
        if cam_view == 0:
            ...
            self.ui.le_cam1_max.setText(str(np.max(img)))
            ...
            gv_camera = self.ui.gv_camera1
        else:
            UpdateManager.cam_2_com = np.asarray(com)
            self.ui.le_cam2_max.setText(str(np.max(img)))
    ...
    def updateCalibrate():
        ...
        #                 self.ui.gv_camera2.setImage(img, autoRange=False, autoLevels=False, autoHistogramRange=False)
    ...
    def update_cam1_settings():
        ...
        if int(ui.cb_SystemSelection.currentIndex()) == 1:
            ...
            cam1_decimate = self.ui.cb_cam1_decimate.isChecked()
            ...
            self.ui.le_cam1_exp_time.setText('%.2f' % (cam_list[cam1_index].exposure_time))
    ...
    def update_cam2_settings():
        ...
        if int(ui.cb_SystemSelection.currentIndex()) == 1:
            ...
            cam2_decimate = self.ui.cb_cam2_decimate.isChecked()
            ...
            self.ui.le_cam2_exp_time.setText('%.2f' % (cam_list[cam2_index].exposure_time))
    ...
    def update_motors():
        ...
        motor1_port = self.ui.cb_motors_1.currentData(0)
        motor2_port = self.ui.cb_motors_2.currentData(0)

        # Determine motor type by index (first 2 are fake cameras, assume all others to be Mightex)
        if self.ui.cb_motors_1.currentIndex < 2:
            motor1_kind = "FakeMotor"
        else:
            motor1_kind = "MDT693AMotor" 

        if self.ui.cb_motors_2.currentIndex < 2:
            motor2_kind = "FakeMotor"
        else:
            motor2_kind = "MDT693AMotor" 

        # Remove previously selected motors
        if len(MotorManager.getMotorList) >= 2:
            MotorManager.removeMotors()

        MotorManager.addMotor(motor1_port, motor1_kind)
        MotorManager.addMotor(motor2_port, motor2_kind)

        # if motor1_index != motor2_index:
        #     motor_list = []
            #Garrison Updated to add "0" inside self.ui.cb_motors_1.currentData(0)
